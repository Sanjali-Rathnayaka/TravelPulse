...
# --- Visualization Sections (Pie, Bar, Map, Wordcloud) ---
...
with st.form("itinerary_form"):
    ...
    start_city = st.text_input("🚐 Start City (e.g., Colombo)", "Colombo")
    end_city = st.text_input("🏁 End City (e.g., Kandy)", "Kandy")
...
if submitted:
    ...
    for day_label, day_df in itinerary_plan:
        ...
        for _, row in day_df.iterrows():
            coords = row['Coordinates'] if isinstance(row['Coordinates'], (list, tuple)) else None
            st.markdown(f"""
    ...
    if not itinerary_plan:
        st.warning("⚠️ No destinations match your preferences.")

# --- Footer ---
st.markdown("---")
st.caption("🔍 Developed for: *Can sentiment analysis of tourist reviews help improve visibility of rural destinations in Sri Lanka through a mobile based recommendation syste
